...
async function loadThemes() {
    try {
        const response = await fetch('https://project-lyhaq.vercel.app/themes.json');
...
async function loadLanguages() {
    try {
        const response = await fetch('https://project-lyhaq.vercel.app/language.json');
        ...
        document.getElementById('languageDropdown').innerHTML = '<div class="loading">Помилка завантаження</div>';
    }
}
...
function renderLanguageDropdown() {
    ...
    dropdown.innerHTML = languages.map(lang => `
        <div class="theme-option ${lang.id === currentLanguage ? 'active' : ''}" onclick="changeLanguage('${lang.id}')">
            ${lang.flag} ${lang.name}
        </div>
    `).join('');
...
function applyLanguage(langId) {
    ...
    document.querySelector('h1').textContent = currentTranslations.appTitle;
    document.getElementById('input').placeholder = currentTranslations.input.placeholder;
    document.getElementById('addBtn').textContent = currentTranslations.input.addButton;
    document.getElementById('achievementsBtn').textContent = `🏆 ${currentTranslations.buttons.achievements}`;
    document.getElementById('themeBtn').textContent = currentTranslations.buttons.theme;
    document.getElementById('languageBtn').textContent = `🌐 ${currentTranslations.buttons.language}`;
...
function applyTheme(themeId) {
    ...
    document.getElementById('languageBtn').style.background = theme.primary;
    document.getElementById('languageBtn').style.color = 'white';
    document.getElementById('languageBtn').style.borderColor = theme.primary;
    document.getElementById('achievementsBtn').style.background = theme.primary;
    document.getElementById('achievementsBtn').style.color = 'white';
    document.getElementById('achievementsBtn').style.borderColor = theme.primary;
    ...
    style.textContent = `
        ...
        #languageBtn:hover { background: ${theme.primaryHover} !important; border-color: ${theme.primaryHover} !important; }
        #achievementsBtn:hover { background: ${theme.primaryHover} !important; border-color: ${theme.primaryHover} !important; }
        ...
        .empty-state { background: ${theme.itemBg} !important; color: ${theme.text} !important; border-color: ${theme.itemBg} !important; }
    `;
...
function updateStats() {
    ...
    const t = currentTranslations.stats || { total: 'Всього', active: 'Активних', completed: 'Завершено' };
    document.getElementById('totalStat').textContent = `${t.total}: ${total}`;
    document.getElementById('activeStat').textContent = `${t.active}: ${active}`;
    document.getElementById('completedStat').textContent = `${t.completed}: ${completed}`;
}
...
function render() {
    ...
    if (todos.length === 0) {
        const emptyMsg = currentTranslations.emptyState?.message || 'Немає завдань. Додайте перше!';
        list.innerHTML = `
        ...
        const deleteBtn = currentTranslations.task?.deleteButton || 'Видалити';
        list.innerHTML = todos.map((todo, i) => `
            <li class="${todo.done ? 'completed' : ''}" draggable="true" data-index="${i}">
                ...
                <button class="delete-btn" onclick="del(${i})">${deleteBtn}</button>
...
const achievementsList = [
    { id: 'first_task', icon: '🎯', requirement: () => achievements.totalCreated >= 1 },
    { id: 'task_master', icon: '📝', requirement: () => achievements.totalCreated >= 10 },
    { id: 'task_legend', icon: '📚', requirement: () => achievements.totalCreated >= 50 },
    { id: 'first_complete', icon: '✅', requirement: () => achievements.totalCompleted >= 1 },
    { id: 'productive', icon: '💪', requirement: () => achievements.totalCompleted >= 10 },
    { id: 'super_productive', icon: '🔥', requirement: () => achievements.totalCompleted >= 25 },
    { id: 'unstoppable', icon: '⚡', requirement: () => achievements.totalCompleted >= 50 },
    { id: 'centurion', icon: '💯', requirement: () => achievements.totalCompleted >= 100 },
    { id: 'streak_3', icon: '🔥', requirement: () => achievements.currentStreak >= 3 },
    { id: 'streak_7', icon: '🌟', requirement: () => achievements.currentStreak >= 7 },
    { id: 'streak_30', icon: '👑', requirement: () => achievements.currentStreak >= 30 }
];
...
function showAchievementNotification(achievement) {
    ...
    const achData = currentTranslations.achievements?.list?.find(a => a.id === achievement.id) || {};
    const notifTitle = currentTranslations.achievements?.notification?.title || '🏆 Нагорода отримана!';

    notification.innerHTML = `
        <div class="notification-icon">${achievement.icon}</div>
        <div class="notification-content">
            <div class="notification-title">${notifTitle}</div>
            <div class="notification-description">${achData.title || ''}</div>
        </div>
    `;
...
function renderAchievements() {
    ...
    const t = currentTranslations.achievements || {};

    document.querySelector('.achievements-header h2').textContent = `🏆 ${t.title || 'Ваші досягнення'}`;
    ...
    document.querySelectorAll('.stat-label')[0].textContent = statLabels.completed || 'Виконано завдань';
    document.querySelectorAll('.stat-label')[1].textContent = statLabels.created || 'Створено завдань';
    document.querySelectorAll('.stat-label')[2].textContent = statLabels.streak || 'Днів поспіль';

    list.innerHTML = achievementsList.map(achievement => {
        const unlocked = achievement.requirement();
        const achData = t.list?.find(a => a.id === achievement.id) || {};
        return `
